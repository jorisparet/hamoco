...
By using the power of [PyAutoGUI](https://pypi.org/project/PyAutoGUI/) to control the mouse, [openCV](https://pypi.org/project/opencv-python/) to process the video feed, and [MediaPipe](https://google.github.io/mediapipe/) to track hands, **hamoco** predicts the nature of a hand pose in real-time thanks to a neural network and uses it to perform various kinds of actions.
...
- `THUMB_SIDE` : triggers vertical scrolling using the first location as scrolling origin. Scrolling up or down is done by moving the hand up or down while keeping the same hand pose ;
